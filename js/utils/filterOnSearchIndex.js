...
document.getElementById('operationType').addEventListener('change', (element) => {
    ...
    console.log('tipo de operación: ',element.target.value)
  })
...
//! tipo de precio
document.getElementById('exampleRadios1').addEventListener('change', mostrarValorTypePrice);
document.getElementById('exampleRadios2').addEventListener('change', mostrarValorTypePrice);
...
document.getElementById("buscar")?.addEventListener("click", async () => {
  ...
  window.open(
    ...
    `/propiedad.html?page=${query.page}&limit=${query.limit}&CodigoUsuarioMaestro=${query.CodigoUsuarioMaestro}&realtorId=${query.realtorId}&statusId=${query.statusId}&operationType=${query.operationType}&typeOfProperty=${query.typeOfProperty}&region=${query.region}&commune=${query.commune}&min_price=${query.min_price}&max_price=${query.max_price}&covered_parking_lots=${query.covered_parking_lots}&bathrooms=${query.bathrooms}&surface_m2=${query.surface_m2}&bedrooms=${query.bedrooms}`
  ); 
